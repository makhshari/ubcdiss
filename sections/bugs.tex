...
To only consider valid bugs, we looked for issued bug reports with only ``closed'' status and with ``bug''', ``defect'', or ``error'' labels.  Moreover, to select only representative IoT repositories for our study, we manually analyzed repositories that have more than five labeled issues or more than 50 closed issues based on the information in their readme page, issued bug reports, and their website (when available). We then excluded projects that were not representative of IoT systems such as user interface, documentation, or outdated repositories. In order to identify if a repository is representative for IoT systems or not, we checked if it covers the IoT stack to deliver use cases that include more than one IoT device. The definition we presented in earlier chapters suggests that IoT is about the interaction of multiple devices, so we exclude projects that could not support this feature. However, we do not exclude device-level or integration libraries as they play a crucial role in a lot of other full-stack IoT projects that use these libraries. Our final project list contained 91 open source IoT repositories to analyze. For five repositories that use custom labels (e.g., ``problems'', ``kind/bug'', ``type : bug''), we manually added their labels to our search keywords. In the end, we collected \emph{5,565 bug reports} from the 91 IoT repositories.
...
We also flagged and discarded issues and PRs that could not represent a bug or a bug-fix such as enhancements or how-to-use questions. We examined the entire discussion among developers, as well as the fix commit data (e.g. the commit message and the code diff) to label each bug report. Usually, if the bug report is not a pull request, developers provide a link to the fixed code that is included in a PR, which resolves the associated bug report. In some cases, the root cause of the bug is a fault in clients' code and there is no need for a fix in the original repository. In these cases, we rely on clients' reports of their issues and their faulty code if provided. However, if this is not the case and still we could not find the associated fix code, we discard the bug report. In the end, we labeled \emph{323 bug reports}. Out of these 323 bug reports, 54 of them are PR, and the rest are regular closed GitHub issues.
...
Our interview questions are available in our replication package~\cite{interviewQs}.
...
Our survey questionnaire is available in our replication package~\cite{surveyQs}.
...
\afterpage{%
 \begin{figure}
 \makebox[\textwidth][c]{\includegraphics[width=1.4\textwidth]{./imgs/tax.pdf}}%
...
\textbf{Configuration faults (CNF)}: Hardware devices and development tools need some particular configuration. Wong provisioning of the configurations are considered CNF faults.
